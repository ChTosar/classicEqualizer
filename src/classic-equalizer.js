class ClassicEqualizer extends HTMLElement {
  ...
  constructor() {
    ...
    const colors = JSON.parse(
      this.getAttribute("colors") ||
        '{"barBgColor": "#222222", "barColor": "#d7f0ff", "barColor2": "#a6c0ba", "barColor3": "#fcb750"}'
    );
  ...
  getAudio() {
    this._audio = this._audio
      ? this._audio
      : this.shadowRoot.getElementById("audio") ||
        document.querySelector(this.getAttribute("audio"));
    ...
    this._audio.addEventListener("play", () => {
      ...
      if (!this.analyser) {
        this.analyser = new (window.AudioContext ||
          window.webkitAudioContext)().createAnalyser();
        this.source = this.analyser.context.createMediaElementSource(
          this._audio
        );
  ...
  groupedFrequencies(dataArray) {
    ...
    const binRanges = this.frequencyRanges.map((freq) =>
      Math.floor((freq / nyquist) * bufferLength)
    );
  ...
  columnsTransform(arr, newSize = this.cols) {
    ...
    for (let i = 0; i < newSize; i++) {
      ...
      if (minIndex === maxIndex) {
        ...
        let valorInterpolado =
          arr[minIndex] * pesoInferior + arr[maxIndex] * pesoSuperior;
  ...
  calculateBars() {
    this.barWidth =
      (this.offsetWidth - this.barsMarginX * 2 * this.cols) / this.cols;
    this.barHeight =
      ((this.height || this.offsetWidth * (9 / 20)) -
        this.barsMarginY * 2 * this.rows) /
      this.rows;
  }
      ...
      for (let col = 0; col < this.cols; col++) {
        let x =
          col *
            (barWidth +
              this.barsMarginX * 2 +
              (this.barsMarginX / this.cols + 1)) -
          1;
        let y =
          this.canvas.height -
          row * (barHeight + this.barsMarginY * 2) -
          barHeight;
  ...
  htmlDraw() {
    ...
    this.domEqualizer.style.height =
      this.height || this.offsetHeight * (9 / 20);
  ...
  getColor(row, col) {
    ...
    if (this.debug) {
      this.shadowRoot.querySelector(".frequencys").innerHTML = dataArray
    ...
    if (dataArrayHeight >= (row + 1) * (255 / this.rows)) {
      ...
      (oldPosHeight > dataArrayHeight ? oldPosHeight : dataArrayHeight) >=
      row * (255 / this.rows)
    ) {
